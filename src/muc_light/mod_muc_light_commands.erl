...
send_message(Domain, RoomName, Sender, Message) ->
    SenderJID = jid:from_binary(Sender),
    mod_muc_light_api:send_message(Domain, RoomName, SenderJID, Message).
...
format_response({ok, Msg}) -> Msg;
format_response({not_exists, Msg}) -> {error, not_found, Msg};
format_response({ResStatus, Msg}) when exist =:= ResStatus;
                                       not_allowed =:= ResStatus;
                                       user_without_room =:= ResStatus ->
    {error, denied, Msg};
format_response({_, Reason}) -> {error, internal, Reason}.
