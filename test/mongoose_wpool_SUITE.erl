...
a_global_riak_pool_is_started(_Config) ->
    ...
    [{ok, PoolName}] = mongoose_wpool:start_configured_pools([#{type => riak, scope => global, tag => default,
                                                                opts => #{workers => 12},
                                                                conn_opts => #{address => "localhost",
                                                                               port => 1805}}]),
...
host_specific_pools_are_preserved(_C) ->
    Pools = [#{type => generic, scope => host, tag => default, opts => #{}, conn_opts => #{}},
             #{type => generic, scope => <<"b.com">>, tag => default, opts => #{workers => 12}, conn_opts => #{}}],
    Expanded = mongoose_wpool:expand_pools(Pools, [<<"a.com">>, <<"b.com">>, <<"c.eu">>]),
    ?assertMatch([#{type := generic, host_type := <<"a.com">>, tag := default, opts := [], conn_opts := #{}},
                  #{type := generic, host_type := <<"c.eu">>, tag := default, opts := [], conn_opts := #{}},
                  #{type := generic, host_type := <<"b.com">>, tag := default, opts := [{workers,12}], conn_opts := #{}}], Expanded).
...
pools_for_different_tag_are_expanded_with_host_specific_config_preserved(_C) ->
    ...
    ?assertMatch([#{type := generic, host_type := <<"a.com">>, tag := default, opts := [], conn_opts := #{}},
                  #{type := generic, host_type := <<"c.eu">>, tag := default, opts := [], conn_opts := #{}},
                  #{type := generic, host_type := <<"b.com">>, tag := default, opts := [{workers,12}], conn_opts := #{}},
                  #{type := generic, host_type := <<"a.com">>, tag := other_tag, opts := [], conn_opts := #{}},
                  #{type := generic, host_type := <<"b.com">>, tag := other_tag, opts := [], conn_opts := #{}},
                  #{type := generic, host_type := <<"c.eu">>, tag := other_tag, opts := [], conn_opts := #{}}], Expanded).

global_pool_is_used_by_default(_C) ->
    Pools = [#{type => generic, scope => global, tag => default, opts => #{}, conn_opts => #{}},
             #{type => generic, scope => <<"a.com">>, tag => default, opts => #{}, conn_opts => #{}}],
    StartRes = mongoose_wpool:start_configured_pools(Pools),
